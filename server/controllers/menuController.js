...
exports.createMenuItem = async (req, res) => {
  ...
  try {
    const { name, description, price, CategoryId, BranchId, status, imageUrl, rating, preparationTime, isSpecial, discountPrice } = req.body;
    const menuItem = await MenuItem.create({ name, description, price, CategoryId, BranchId, status, imageUrl, rating, preparationTime, isSpecial, discountPrice });
    ...
    menuItemLogger.info('Menu item created successfully', { req: requestInfo, performedBy });

    res.status(201).json(menuItem);
  } catch (error) {
    menuItemLogger.error('Error creating menu item', { req: requestInfo, error });
...
exports.updateMenuItem = async (req, res) => {
  ...
  try {
    ...
    menuItemLogger.info('Menu item updated successfully', { req: requestInfo, performedBy });

    res.json(menuItem);
  } catch (error) {
    menuItemLogger.error('Error updating menu item', { req: requestInfo, error });
    res.status(400).json({ error: error.message });
  }
};
...
exports.deleteMenuItem = async (req, res) => {
  ...
  try {
    ...
    menuItemLogger.info('Menu item deleted successfully', { req: requestInfo, performedBy });

    res.json({ message: 'Menu item deleted successfully' });
  } catch (error) {
    menuItemLogger.error('Error deleting menu item', { req: requestInfo, error });
    res.status(400).json({ error: error.message });
  }
};
...
exports.getMenuByOther = async (req, res) => {
  try {
    ...
    menuItemLogger.error('Error fetching menu items by category or branch', { error });
    res.status(400).json({ error: error.message });
  }
};
